...
<!-- markdownlint-disable MD013 MD029 --><!-- internal working plan: long prose lines and continuous step numbering are intentional -->
...
| B1-B5 red-team evaluation  | Harness closed; full matrix pending      | Runner now calls the real HIVE graph and records exchanges, duration, tiers, HVI precision/recall/F1, verdict accuracy, guardrail flags, explicit bot-assertion detection, sandbox outcomes, chain validity, attachment metadata, and evidence verification. A versioned 19-scenario by four-persona command can produce 76 raw JSON/CSV runs, including image, inert document, and inert APK fixtures based on Malaysian scam archetypes. Retained evaluation/demo records exercise the implemented modes, but the complete matrix against configured cloud models still requires time and retained outputs. |
