...
export function App() {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileNav?"open":""}`}><header><Logo/><div><strong>HIVE</strong><span>Control panel</span></div><button className="icon-button mobile-only" aria-label="Close navigation" onClick={()=>setMobileNav(false)}>×</button></header><nav>{navigation.map(([name,label])=>name==="divider"?<hr key={name}/>:<div key={name}><button className={route.name===name?"active":""} onClick={()=>navigate(name)}><i/><span>{label}</span>{name==="cases"&&<b>{(sessions.data?.length||0)+(history.data?.length||0)}</b>}</button>{name==="cases"&&route.name==="case"&&<div className="case-rail">{sessions.data?.map(item=><button key={item.peer_id} className={String(item.peer_id)===route.id?"active":""} onClick={()=>navigate(`case/live/${item.peer_id}/conversation`)}><span>{item.peer_id}</span><strong>{typeof item.score==="number"?item.score.toFixed(2):"—"}</strong></button>)}</div>}</div>)}</nav><button className={`setup-entry ${allReady?"healthy":"warning"}`} onClick={()=>setSetupOpen(true)}><span className={`status-dot ${allReady?"healthy":"warning"}`}/><span><strong>Setup</strong><small>Models · Telegram · Security</small></span><b>{readiness}</b></button></aside>
    <div className="main-shell"><header className="topbar"><button className="icon-button mobile-only" aria-label="Open navigation" onClick={()=>setMobileNav(true)}>≡</button><span className="mobile-brand mobile-only"><Logo/><strong>HIVE</strong></span><button className="breadcrumb" onClick={()=>route.name==="case"&&navigate("cases")}><span>{route.name==="case"?"Cases":"HIVE"}</span><b>/</b><strong>{route.name==="case"?route.id:titleCase(route.name)}</strong></button>{route.name==="case"&&route.source==="live"&&sessions.data?.length?<CaseStepper route={route} rows={sessions.data} navigate={navigate}/>:null}<button className="command-trigger" onClick={()=>setPaletteOpen(true)}><span>Search or run an action</span><kbd>Ctrl K</kbd></button><Chip tone={runtime.data?.running?"success":runtime.data?.ready?"neutral":"warning"}><span className={`status-dot ${runtime.data?.running?"live":""}`}/>{runtime.data?.running?"Agent running":runtime.data?.ready?"Agent stopped":"Setup required"}</Chip></header><main id="workspace">{renderRoute(route,navigate,()=>setSetupOpen(true))}</main></div>
...
function CaseStepper({route,rows,navigate}:{route:Route;rows:CaseSummary[];navigate:(route:string)=>void}){const index=Math.max(0,rows.findIndex(row=>String(row.peer_id)===route.id));const tab=window.location.hash.split("/")[4]||"conversation";const move=(offset:number)=>{const next=rows[(index+offset+rows.length)%rows.length];if(next?.peer_id)navigate(`case/live/${next.peer_id}/${tab}`);};return <div className="case-stepper"><button aria-label="Previous case" onClick={()=>move(-1)}>‹</button><span>{index+1} of {rows.length}</span><button aria-label="Next case" onClick={()=>move(1)}>›</button></div>;}
...
function ToastStack({rows,dismiss}:{rows:Toast[];dismiss:(id:number)=>void}){return <aside className="toast-stack" aria-live="polite">{rows.map(row=><article className={`toast ${row.kind}`} key={row.id}><span className={`status-dot ${row.kind}`}/><div><strong>{row.title}</strong>{row.detail&&<p>{row.detail}</p>}</div><button aria-label="Dismiss notification" onClick={()=>dismiss(row.id)}>×</button></article>)}</aside>}
