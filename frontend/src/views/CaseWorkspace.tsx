...
export function CaseWorkspace({ source, id, navigate }: { source: "live" | "history"; id: string; navigate: (route: string) => void }) {
  ...
  async function changePersona(persona: string) { setBusy(true); try { await post(`/api/sessions/${id}/persona`, {persona}); setOperationNotice(`Persona changed to ${titleCase(persona)}.`); await refresh(); } catch(reason) { window.alert(reason instanceof Error ? reason.message : String(reason)); } finally { setBusy(false); } }
  async function reanalyze() { setBusy(true); setOperationNotice("Reanalysis queued. The sealed record will not be modified."); try { const job=await post<JsonRecord>(`/api/history/${id}/reanalyze`); const jobId=String(job.id || ""); for(let attempt=0;attempt<180;attempt+=1){await new Promise(resolve=>window.setTimeout(resolve,1000));const status=await api<JsonRecord>(`/api/reanalysis/${jobId}`);if(status.status==="completed"){setOperationNotice("Reanalysis completed as a new dated analysis run. The sealed record is unchanged.");await analysisRuns.refresh();return;}if(status.status==="failed")throw new Error(String(status.error || "Reanalysis failed"));}throw new Error("Reanalysis is still running. Check Activity for completion."); } catch(reason) { setOperationNotice(reason instanceof Error ? reason.message : String(reason)); } finally { setBusy(false); } }
  ...
  return <div className="view workspace-view">
    <PageHeading eyebrow={`${source === "live" ? "Live takeover" : "Sealed case"} · ${data.peer_id || id}`} title={data.name || `Peer ${data.peer_id || id}`} copy={`${titleCase(data.persona)} · ${data.messages?.length || data.message_count || 0} messages · ${formatDate(data.started_ts, true)}`} actions={<><Chip tone={verdictTone(data.verdict)}>{titleCase(data.verdict || "not assessed")}</Chip><Score value={risk}/>{source === "live" ? <><select className="persona-select" aria-label="Change persona" defaultValue={data.persona} disabled={busy} onChange={event=>void changePersona(event.target.value)}><option value="confused_elderly">Confused elderly</option><option value="naive_young_adult">Naive young adult</option><option value="overseas_worker">Overseas worker</option><option value="small_business_owner">Small business owner</option></select><Button tone="danger" onClick={() => setConfirmStop(true)}>Stop & seal</Button></> : <Button disabled={busy} onClick={()=>void reanalyze()}>{busy?"Reanalysing…":"Reanalyse"}</Button>}</>} />
    {source === "history" && <div className="archive-bar"><strong>Sealed record</strong><span>Reanalysis creates a new dated run and never rewrites this transcript.</span><Chip tone="success">Read-only</Chip></div>}
    {operationNotice && <div className="inline-notice">{operationNotice}</div>}
